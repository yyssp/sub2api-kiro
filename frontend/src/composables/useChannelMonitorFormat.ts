...
export function useChannelMonitorFormat() {
  ...
  function providerBadgeClass(p: Provider | string): string {
    switch (p) {
      ...
      // kiro=violet / cursor=amber / kimi=pink / zhipu=indigo / deepseek=teal。
  ...
  function providerPickerClass(p: Provider | string, active: boolean): string {
    switch (p) {
      ...
      case PROVIDER_CURSOR:
        return active
          ? 'border-amber-500 bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300 dark:border-amber-400'
