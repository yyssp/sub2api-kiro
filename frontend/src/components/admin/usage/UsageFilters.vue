      ...
      <div class="flex flex-1 flex-wrap items-end gap-4">
            ...
            >
              <span
                >{{ u.email
                  ...
                  >（{{ t("admin.usage.userDeletedBadge") }}）</span
                ></span
              >
        ...
        <div v-if="mode === 'usage'" class="w-full sm:w-auto sm:min-w-[220px]">
          <label class="input-label">{{
            t("admin.usage.upstreamModelAudit")
          }}</label>
          <Select
        ...
        <!-- Cache Strategy Filter -->
        <div class="w-full sm:w-auto sm:min-w-[200px]">
          <label class="input-label">{{
            t("admin.usage.cacheStrategy")
          }}</label>
          <Select
            v-model="filters.cache_strategy_id"
...
const cacheStrategyOptions = ref<SelectOption[]>([
  { value: null, label: t("admin.usage.allCacheStrategies") },
]);
...
const errorCategoryOptions = computed<SelectOption[]>(() => [
  ...
  ...errorCategoryCodes.map((c) => ({
    value: c,
    label: t("usage.errors.categories." + c),
  })),
]);
...
onMounted(async () => {
  ...
  try {
    // 缓存策略列表独立 try：策略接口失败不应连带把分组下拉也清空。
    const strategies = await adminAPI.cacheStrategies.list();
    cacheStrategyOptions.value.push(
      ...strategies.map((item: any) => ({ value: item.id, label: item.name })),
    );
  } catch {
    // Ignore filter option loading errors (page still usable)
  }
