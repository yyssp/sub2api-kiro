export default {
    accounts: {
      ...
      kiroImportSourceKiroRs: 'kiro.rs 凭证',
      kiroImportSourceKiroIde: 'Kiro IDE 导出',
      kiroImportHintKiroRs: '支持 kiro.rs 的 credentials.json：单个对象、数组（多账号），以及每行 `ksk_xxx|region` 的纯文本清单。缺失的 authMethod、machineId、region、endpoint 会按 kiro.rs 的默认规则自动补齐。',
      kiroImportHintKiroIde: '支持 Kiro IDE 导出的凭证 JSON，需要同时包含 accessToken、refreshToken 与运行时元数据。',
      kiroImportFile: '凭证文件',
