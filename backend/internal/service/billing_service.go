...
// CalculateCostWithLongContext preserves the historical marginal long-context
// rule used by the compatibility API. New pricing data should use
// calculateCostInternalWithPolicy so catalog tiers remain the source of truth.
func (s *BillingService) CalculateCostWithLongContext(model string, tokens UsageTokens, rateMultiplier float64, threshold int, extraMultiplier float64) (*CostBreakdown, error) {
	if threshold <= 0 || extraMultiplier <= 1 {
		return s.CalculateCost(model, tokens, rateMultiplier)
	}
	totalInput := tokens.CacheReadTokens + tokens.InputTokens
	if totalInput <= threshold {
		return s.CalculateCost(model, tokens, rateMultiplier)
	}

	inRangeCache := tokens.CacheReadTokens
	inRangeInput := tokens.InputTokens
	if inRangeCache > threshold {
		inRangeCache = threshold
		inRangeInput = 0
	} else if inRangeCache+inRangeInput > threshold {
		inRangeInput = threshold - inRangeCache
	}
	outRangeCache := tokens.CacheReadTokens - inRangeCache
	outRangeInput := tokens.InputTokens - inRangeInput

	inRangeCost, err := s.CalculateCost(model, UsageTokens{
		InputTokens:           inRangeInput,
		ImageInputTokens:      tokens.ImageInputTokens,
		OutputTokens:          tokens.OutputTokens,
		CacheCreationTokens:   tokens.CacheCreationTokens,
		CacheCreation5mTokens: tokens.CacheCreation5mTokens,
		CacheCreation1hTokens: tokens.CacheCreation1hTokens,
		CacheReadTokens:       inRangeCache,
	...
	outRangeCost, err := s.CalculateCost(model, UsageTokens{
		InputTokens:     outRangeInput,
		CacheReadTokens: outRangeCache,
	}, rateMultiplier*extraMultiplier)
	if err != nil {
		return nil, fmt.Errorf("out-range cost: %w", err)
	}
	return &CostBreakdown{
