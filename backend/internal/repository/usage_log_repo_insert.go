...
var usageLogInsertArgTypes = [...]string{
	...
	"bigint",      // group_id
	"bigint",      // cache_strategy_id
	"text",        // cache_strategy_name
...
func buildUsageLogBatchInsertQuery(keys []string, preparedByKey map[string]usageLogInsertPrepared) (string, []any) {
	...
	// Each batch row prepends the synthetic input_index before the 64 usage-log values.
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	return usageLogInsertPrepared{
		...
		args: []any{
			...
			groupID,
			nullInt64(log.CacheStrategyID),
			nullString(log.CacheStrategyName),
			subscriptionID,
			log.InputTokens,
			log.OutputTokens,
