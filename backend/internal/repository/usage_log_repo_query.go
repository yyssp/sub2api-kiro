...
// Keep this list in lockstep with scanUsageLog and usageLogInsertArgTypes.
// The local cache-strategy/Kiro fields and upstream reasoning/compaction fields
// are intentionally retained together so historical rows remain readable.
const usageLogSelectColumns = "id, user_id, api_key_id, account_id, request_id, model, requested_model, upstream_model, upstream_response_model, upstream_model_mismatch, group_id, cache_strategy_id, cache_strategy_name, subscription_id, input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens, cache_creation_5m_tokens, cache_creation_1h_tokens, image_output_tokens, image_output_cost, image_input_tokens, image_input_cost, input_cost, output_cost, cache_creation_cost, cache_read_cost, total_cost, actual_cost, rate_multiplier, account_rate_multiplier, billing_type, request_type, stream, openai_ws_mode, duration_ms, first_token_ms, user_agent, ip_address, image_count, image_size, image_input_size, image_output_size, image_size_source, image_size_breakdown, video_count, video_resolution, video_duration_seconds, service_tier, reasoning_effort, requested_reasoning_effort, inbound_endpoint, upstream_endpoint, cache_ttl_overridden, long_context_billing_applied, channel_id, model_mapping_chain, billing_tier, billing_mode, account_stats_cost, kiro_credits, usage_source, upstream_request_id, session_id, native_compaction_v2, created_at"
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		cacheStrategyID           sql.NullInt64
		cacheStrategyName         sql.NullString
	...
	if cacheStrategyID.Valid {
		value := cacheStrategyID.Int64
		log.CacheStrategyID = &value
	}
	if cacheStrategyName.Valid {
		value := cacheStrategyName.String
		log.CacheStrategyName = &value
	}
