...
// service.monitorProviders（internal/service/channel_monitor_validate.go）应与
// 之保持一致：minimax 曾漏进 service 校验表（只能写库不能建监控），已补齐。
// 约束取超集不会漏放行，反之则会让 provider 无法落库。
// cursor 与 kiro 同为「仅配额、无探活 adapter」的 provider。
