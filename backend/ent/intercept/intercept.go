...
// The CacheStrategyFunc type is an adapter to allow the use of ordinary function as a Querier.
type CacheStrategyFunc func(context.Context, *ent.CacheStrategyQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f CacheStrategyFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.CacheStrategyQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.CacheStrategyQuery", q)
}

// The TraverseCacheStrategy type is an adapter to allow the use of ordinary function as Traverser.
type TraverseCacheStrategy func(context.Context, *ent.CacheStrategyQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseCacheStrategy) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseCacheStrategy) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.CacheStrategyQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.CacheStrategyQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.CacheStrategyQuery:
		return &query[*ent.CacheStrategyQuery, predicate.CacheStrategy, cachestrategy.OrderOption]{typ: ent.TypeCacheStrategy, tq: q}, nil
